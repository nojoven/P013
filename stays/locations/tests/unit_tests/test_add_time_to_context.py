...
class TestAddTimeToContext:
    def test_add_time_to_context(self):
        ...
        json_file_path_time = os.path.join(base_dir, 'utils', 'ninjas_api_time_paris_response.json')
        ...
        # Check the result
        assert "Local_Time" in result and result["Local_Time"] == time_response["datetime"]
        assert "Time_Zone" in result and result["Time_Zone"] == time_response["timezone"]
