...
class TestAddWeatherToContext:
    def test_add_weather_to_context(self):
        ...
        assert "Sunrise" in result and isinstance(datetime.strptime(result["Sunrise"], "%H:%M:%S"), datetime)
        assert "Sunset" in result and isinstance(datetime.strptime(result["Sunset"], "%H:%M:%S"), datetime)
