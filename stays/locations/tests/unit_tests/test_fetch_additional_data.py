...
from locations.tests.helpers import FakeResponse, MockCache
...
from locations.tests.datasets import VALID_CAPITALS, INVALID_CAPITALS, HEADERS
...
async def test_fetch_additional_data_cache_miss(mocker):
    ...
    mock_get = mocker.patch('httpx.AsyncClient.get', new=async_mock)
...
async def test_fetch_additional_data_cache_hit(mocker):
    ...
    mock_cache.get.return_value = [FakeResponse(200, {'data': 'additional_data'})] * 3  # Simule des données déjà présentes dans le cache
