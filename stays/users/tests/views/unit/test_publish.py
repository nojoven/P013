...
class TestPublishView(TestCase):
    ...
    def test_reach_publish_page_with_authenticated_user(self, mock_update_user_status):
        ...
        self.assertTrue(
            response.templates[0].name == "publish.html"
        )  # Check the template
    ...
    def test_publish_view_text_story(self, mock_update_user_status):
        ...
        self.assertEqual(
            publication.country_code_of_stay, form_data["country_code_of_stay"]
        )
        self.assertEqual(
            publication.published_from_country_code.code,
            form_data["published_from_country_code"],
        )
