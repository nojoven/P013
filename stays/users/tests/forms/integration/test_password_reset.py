...
class TestPasswordResetForm(TestCase):
    ...
    def test_sending_values(self):
        assert all((EMAIL_HOST_USER, MAILGUN_API_KEY, MAILGUN_DOMAIN_NAME, DEFAULT_EMAIL_DESTINATION))
        assert all((isinstance(value, str) for value in (EMAIL_HOST_USER, MAILGUN_API_KEY, MAILGUN_DOMAIN_NAME, DEFAULT_EMAIL_DESTINATION)))
