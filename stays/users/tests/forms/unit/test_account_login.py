...
class TestAccountLoginForm(TestCase):
    ...
    def test_form_valid(self):
        form = AccountLoginForm(data={'username': 'testuser@example.com', 'password': 'password'})
    ...
    def test_form_invalid_email(self):
        form = AccountLoginForm(data={'email': 'wronguser@example.com', 'password': 'password'})
        self.assertFalse(form.is_valid())
        self.assertIn('__all__', form.errors)
    ...
    def test_form_invalid_password(self):
        form = AccountLoginForm(data={'username': 'testuser@example.com', 'password': 'wrongpassword'})
        self.assertFalse(form.is_valid())
        self.assertIn('__all__', form.errors)
        error_message = form.errors['__all__'][0]  # Prenez le premier message d'erreur
