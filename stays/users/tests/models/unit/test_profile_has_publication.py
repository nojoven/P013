...
def test_profile_has_publication(db, mock_profile, mock_publication):
    ...
    profile_has_publication = baker.make(
        ProfileHasPublication,
        user_profile=mock_profile,
        publication_of_user=mock_publication,
    )
...
def test_delete_profile_has_publication(db, mock_profile, mock_publication):
    ...
    assert ProfileHasPublication.objects.filter(
        user_profile=mock_profile, publication_of_user=mock_publication
    ).exists()
    ...
    assert not ProfileHasPublication.objects.filter(
        user_profile=mock_profile, publication_of_user=mock_publication
    ).exists()
...
@pytest.mark.django_db
def test_delete_profile_deletes_profile_has_publication(
    db, mock_profile, mock_publication
):
    # Désactivez le signal create_profile_has_publication
    post_save.disconnect(create_profile_has_publication, sender=Publication)
    # Supprimez tous les objets ProfileHasPublication
...
@pytest.mark.django_db
def test_delete_publication_deletes_profile_has_publication(
    db, mock_profile, mock_publication
):
    # Désactivez le signal create_profile_has_publication
    post_save.disconnect(create_profile_has_publication, sender=Publication)
