...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            if (
                "No changes detected" in makemigrations_output
                and "No migrations to apply" in migrate_output
            ):
                self.stdout.write("No migrations necessary.")
            elif (
                "Migrations for" in makemigrations_output
                and "Applying" in migrate_output
            ):
        ...
        if os.getenv("RUNNING_FROM_TEST"):
            ...
            try:
                ...
                self.stdout.write(
                    f"Error in model or database Cities light Country: {str(e)}"
                )
        else:
            try:
            ...
            for other_profile in profiles:
                if profile != other_profile:
                    Follow.objects.add_follower(
                        follower=profile, followee=other_profile
                    )
