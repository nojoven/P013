...
class PublicationEditFormTest(WebTest, TestCase):
    ...
    def test_form_with_valid_text_data(self, mock_update_user_status):
        response = self.app.get(reverse('core:edit_publication', args=[self.text_publication.uuid]), user=self.profile)
        ic(response)
        ic(response.forms)
        form = response.forms[0]  # replace 'form' with the actual form name if it's different
        ic(form.fields.keys())
        ...
        response = form.submit(upload_files=[('picture', 'new_pic.png', png_content, 'image/png')])


        self.assertRedirects(response, reverse('core:publication', args=[self.text_publication.uuid]))
    ...
    def test_form_with_valid_voice_data(self, mock_update_user_status):
        ...
        response = form.submit(upload_files=[('audio_file', 'new_file.mp3', mp3_content, 'audio/mpeg')])
