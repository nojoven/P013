...
class TestCheckUserUpvotedPublicationView(WebTest, TestCase):
    ...
    def test_upvote(self, mock_update_user_status):
        self.assertFalse(
            PublicationUpvote.objects.filter(
                publication=self.voice_publication, upvote_profile=self.profile.slug
            ).exists()
        )
    ...
    def test_unvote(self, mock_update_user_status):
        ...
        self.assertFalse(
            PublicationUpvote.objects.filter(
                publication=self.text_publication, upvote_profile=self.profile.slug
            ).exists()
        )
