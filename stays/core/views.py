...
def check_user_upvoted_publication(request, uuid):
    ...
    has_upvoted = PublicationUpvote.objects.filter(
        publication__uuid=uuid, upvote_profile=viewer
    ).exists()
...
def toggle_upvote(request, uuid):
    ...
    upvote, created = PublicationUpvote.objects.get_or_create(
        publication=publication, upvote_profile=profile.slug
    )
...
class PublicationDetailView(LoginRequiredMixin, NeverCacheMixin, DetailView):
    ...
    def get_object(self):
        ...
        author_profile_picture = get_author_picture_from_slug(
            publication.author_slug, Profile
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            has_upvoted = PublicationUpvote.objects.filter(
                publication=publication.uuid, upvote_profile=self.request.user.slug
            ).exists()
        else:
            has_upvoted = False
        ...
        total_upvotes_count = PublicationUpvote.objects.filter(
