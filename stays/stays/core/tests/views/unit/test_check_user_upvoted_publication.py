...
class TestCheckUserUpvotedPublicationView(WebTest, TestCase):
    ...
    def test_check_user_upvoted_publication(self, mock_update_user_status):
        ...
        baker.make(PublicationUpvote, publication=self.voice_publication, upvote_profile=self.profile.slug)

        # Call the check_user_upvoted_publication view
        response = self.client.get(reverse('core:check_user_upvoted', args=[str(self.voice_publication.uuid)]))
    ...
    def test_check_new_user_not_upvoted_publication(self, mock_update_user_status):
        ...
        response = self.client.get(reverse('core:check_user_upvoted', args=[str(self.text_publication.uuid)]))
