...
class TestHomeView(TestCase):
    ...
    def test_home_view(self):
        ...
        assert response.context['page_obj'][0].title in self.publications.values_list('title', flat=True)
    ...
    def test_home_view_page_3(self):
        ...
        assert response.status_code == 200
        assert 'page_obj' in response.context
        assert len(response.context['page_obj']) == 2  # Pas de publications sur cette page
...
# @pytest.mark.django_db
# class TestEmptyHomeView(TestCase):
#     def setUp(self):
...
#     def test_home_view_with_no_publications(self):
#         # Delete all Publication instances
#         # Test with page=1
#         # Check that the session variable has been reset
#         session = self.client.session
#         session['force_renew_session'] = True
