...
def publication(profile):
    return baker.make(Publication, author_slug=profile.slug, author_username=profile.username)
...
def form_data(publication, profile):
    return {
        ...
        'summary': fake.text(),
        'picture': SimpleUploadedFile('profile_pic.jpg', b'file_content', content_type='image/jpeg'),
        'year_of_stay': fake.year(),
        'season_of_stay': 'Spring',
        'text_story': fake.text(),
        'voice_story': SimpleUploadedFile('voice_story.mp3', b'file_content', content_type='audio/mpeg'),
    }
