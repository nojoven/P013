...
class TestProfileModel:
    ...
    def test_get_absolute_url(self):
        assert self.user1.get_absolute_url() == reverse('users:account', args=[self.user1.slug])

    def test_save(self):
        self.user1.email = "newemail@example.com"
        self.user1.save()
        assert self.user1.slug == slugify(f"{self.user1.email.split('@')[0]}{self.user1.uuid}")
...
class TestCrudProfileModel:
    ...
    def test_modify_profile(self):
        ...
        assert self.profile.slug == slugify(f"{new_email.split('@')[0]}{self.profile.uuid}")
