...
class TestAccountDetailsView(TestCase):
    ...
    def test_account_details_with_authenticated_user(self, mock_update_user_status, mock_mixin):
        # ic(self.user.is_authenticated)
        # ic(User.objects.get(email=self.user.email).slug)
        # ic(User.objects.get(email=self.user.email).username)
        # ic(User.objects.get(email=self.user.email).password)
        # self.client.login(email=self.user.email, password='testpasswordP9671!')  # Log in the user
        self.client.force_login(self.user)
        # ic(self.url)
        # self.user.is_athenticated = True
        ...
        # ic(response.location)
        self.assertIn('current_user', response.context)  # Check the context
        self.assertEqual(response.context['current_user'], self.user)  # Check the current user
